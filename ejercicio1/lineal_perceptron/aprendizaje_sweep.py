"""Runner paralelo para experimentos de APRENDIZAJE — perceptrón lineal.

Entrena el perceptrón lineal sobre TODO el dataset (sin train/test split, sin
...
# IMPORTANTE: setear env vars de BLAS ANTES de importar numpy. macOS spawn
# re-ejecuta el módulo en cada worker, así que los children las heredan.
import os
...
from linear_perceptron import (  # noqa: E402
...
def _prepare_data(csv_path: Path, sweep: dict
    ...
    """Carga CSV, z-score sobre TODO el dataset (sin split), devuelve
    (df_normalizado, feature_cols, var_z) para que R² = 1 - MSE/var_z."""
...
def run_combo(csv_path_str: str, sweep_json: str, lr: float, seed: int):
    """Worker: corre UNA combinación (lr, seed). Top-level por picklability."""
...
def main() -> None:
    ...
    # Summary: por LR, mean ± std del último epoch sobre seeds
    ...
    # Tabla de parámetros — un fila por combo
    params_rows = []
    for lr in sweep["learning_rates"]:
        for seed in sweep["seeds"]:
            params_rows.append({

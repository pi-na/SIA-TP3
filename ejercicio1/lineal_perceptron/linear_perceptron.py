"""Perceptrón lineal multi-feature con K-fold estratificado.

Entrena un Adaline (activación identidad) para replicar el output continuo
...
Pipeline:
    ...
    3. Para cada fold: normaliza features (z-score, fit-on-train-only),
       entrena el perceptrón online, evalúa MSE + métricas de clasificación.
    4. Escribe 3 CSVs en `output/<model_name>_<timestamp>/`:
...
Ver `informe_implementacion.md` para las decisiones de diseño.
"""
...
def load_config(path: Path) -> dict:
    ...
    if config["normalization"] != "zscore":
        raise NotImplementedError(
            ...
            "Sólo 'zscore' está soportado en esta iteración."
        )
    return config
...
def load_csv(csv_path: Path, config: dict) -> tuple[pd.DataFrame, list[str]]:
    ...
    feature_cols = todas las columnas numéricas excepto target_col, eval_col,
    ...
    if target_col not in df.columns:
        raise ValueError(f"target_col='{target_col}' no está en el CSV.")
    if eval_col not in df.columns:
        raise ValueError(f"eval_col='{eval_col}' no está en el CSV.")

    # Warning sobre exclude_features inexistentes (permisivo, no falla)
    missing_excludes = exclude - set(df.columns)
    if missing_excludes:
        print(f"WARNING: exclude_features ignoradas (no están en el CSV): "
...
def make_stratified_folds(
    ...
    Cada fold conserva la proporción original de clases.
    """
...
def train_perceptron(
    ...
    """Adaline online learning multi-feature.

    Returns:
        weights: shape (n_features + 1,) — weights[0] es bias.
        mse_history: lista con MSE de train al final de cada época.
    """
    ...
    for epoch in range(epochs):
        for mu in range(P):
            O = weights @ X[mu]
            weights += learning_rate * (z[mu] - O) * X[mu]
...
def compute_metrics(
    ...
    """MSE (regresión vs target_col) + métricas de clasificación (vs eval_col).

    Devuelve dict con: mse, tp, fp, fn, tn, accuracy, precision, recall, f1,
    tpr, fpr.
    """
    P = len(df)
...
def train_and_test_fold(
    ...
    feature_cols: list[str], config: dict, fold_seed: int,
) -> tuple[dict, list[float], np.ndarray]:
    """Entrena en train_df, evalúa en test_df. Devuelve (metrics, mse_hist, w)."""
    target_col = config["target_col"]
    eval_col = config["eval_col"]
    ...
    return fold_metrics, mse_history, weights


def _run_fold_worker(args):
    """Worker para multiprocessing.Pool. Devuelve (fold_idx, metrics, hist, w).

    Top-level para que sea picklable en macOS (spawn).
    """
    fold_idx, train_df, test_df, feature_cols, config, fold_seed = args
    fold_metrics, mse_history, weights = train_and_test_fold(
        train_df, test_df, feature_cols, config, fold_seed,
    )
    return fold_idx, fold_metrics, mse_history, weights
...
def train_and_test_dataset(
    ...
    """K-fold completo. Devuelve (metrics_df, mse_history_df, weights_df).

    Args:
        workers: cantidad de procesos paralelos. None = k_folds (paralelo total).
                 1 = ejecución serial (útil para debugging).
    """
    k = config["k_folds"]
    ...
    fold_args = [
        (k_i, train_df, test_df, feature_cols, config, seed + k_i)
    ...
    all_history = []
    all_weights = []
    for fold_idx, fold_metrics, mse_hist, weights in results:
    ...
    # Filas resumen: mean / std sobre columnas numéricas (excepto fold)
    ...
    return metrics_df, mse_history_df, weights_df
...
def main():
    ...
    metrics_df, mse_history_df, weights_df = train_and_test_dataset(

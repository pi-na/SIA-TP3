"""K-fold sweep — perceptrón no-lineal (sigmoide).
...
Uso:
    python kfold_sweep_nonlinear.py [--config PATH]

Config por defecto: nonlinear_perceptron/analisis_outputs/kfold_sweep/config.json

Nota: seed-std≈0 en el sweep multi-seed → se usa una sola seed.
"""
...
DEFAULT_CFG = ROOT / "nonlinear_perceptron" / "analisis_outputs" / "kfold_sweep" / "config.json"
OUT_ROOT    = ROOT / "nonlinear_perceptron" / "analisis_outputs" / "kfold_sweep"
...
def make_run_config(base_cfg_path: Path, k: int, seed: int, epochs: int,
    ...
    tmp = Path(tempfile.mkstemp(suffix=".json", prefix=f"kfold_nl_k{k}_")[1])
...
def run_k(base_cfg_path: Path, k: int, seed: int, epochs: int,
    ...
    model_name = f"kfold_sweep_nl_k{k}"
...
def main() -> None:
    ...
    print(f"=== K-fold sweep — no-lineal (K ∈ {k_values}, seed={seed}, "
    ...
    print("\nPara generar el plot: python plot_kfold_sweep.py --perceptron nonlinear")

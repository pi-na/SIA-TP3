...
\centering
\includegraphics[width=0.78\linewidth]{../ejercicio0/presentacion/xor_convergencia.png}\\[0.4em]

\begin{insight}
\textbf{El XOR no es separable linealmente.} Necesita capa oculta no lineal. Las dos arquitecturas convergen: \(\boldsymbol{[2,2,1]}\) (mínima) y \(\boldsymbol{[2,3,2,1]}\) (con capa extra). \textbf{Eje y en escala logarítmica.}
\end{insight}
\end{frame}
...
\centering
\includegraphics[width=\linewidth]{../ejercicio1/nonlinear_perceptron/analisis_outputs/roc_curve.png}\\
\textcolor{muted}{\footnotesize \textbf{AUC-ROC = 0.9921}\\(lineal: 0.9720)}

\column{0.5\linewidth}
\centering
\includegraphics[width=\linewidth]{../ejercicio1/nonlinear_perceptron/analisis_outputs/pr_curve.png}\\
\textcolor{muted}{\footnotesize PR curve más informativa que ROC para clases desbalanceadas (fraude $\approx 12\%$)}
\end{columns}
\vspace{0.4em}
...
\begin{takeaway}
\textbf{El modelo entrega scores. La política de threshold es decisión de negocio:}\\[0.2em]
\begin{itemize}[leftmargin=1.4em]
  \item Auditoría manual barata $\Rightarrow$ threshold $\approx 0.89$.
  \item Falsos positivos costosos $\Rightarrow$ threshold $\geq 0.95$.
\end{itemize}
\end{takeaway}
\end{frame}
...
\normalsize
\begin{warning}
\textbf{Interpretación: distribution shift, no underfitting.}\\
...
\centering
\includegraphics[width=0.55\linewidth]{../ejercicio2/presentacion/08_confusion_matrix_final.png}\\[0.4em]

\begin{takeaway}
\textbf{Patrón de distribution shift:} confusiones distribuidas, no concentradas en una clase.\\[0.3em]

$\Rightarrow$ Si el problema es shift y no falta de capacidad, lo que necesitamos no es un modelo más grande. \\
\textbf{Necesitamos más datos que cubran la distribución del test.}\\[0.3em]

\textcolor{muted}{Y eso es justamente lo que el Ej 3 nos provee.}
\end{takeaway}
\end{frame}
...
\begin{itemize}
  \item \textbf{Dropout:} gana en val ($+0.44$pp), \emph{pierde} en test ($-0.08$pp). Reduce varianza al fold, no al shift.
  \item \textbf{wider} \([784,200,100,10]\): mejor en val, peor en test. Más capacidad memoriza el train. \textbf{Descarta que falte capacidad.}
  \item \textbf{Combinar todo}: L2+dropout+aug peor que L2+aug solo. La regularización compite consigo misma cuando se exagera.
\end{itemize}
\end{warning}
\end{frame}
...
\centering
\Large \textcolor{warn}{\textbf{Mejor: 96.88\%. Faltan 1.12 pp.}}\\[1em]

\normalsize\raggedright
\textbf{Tres hipótesis ordenadas por probabilidad:}
\begin{enumerate}
  \item \textbf{Augmentation insuficiente.}\\
        Gaussian noise es \emph{isotrópico} (independiente por píxel). El shift parece ser de \emph{estilo de escritura}: rotación pequeña, traslación, grosor de trazo. Pide augmentation \textbf{geométrica}, no por noise. No la implementamos en este TP.
...
\begin{takeaway}
\textbf{Próximo paso}: augmentation por rotación/traslación pequeñas; reconsiderar arquitectura solo después.
\end{takeaway}
\end{frame}
